...
void* smoker_tobacco_routine(void* arg){
	...
	if(sem_trywait(&matches) == 0){
		...
		printf("SMOKER_WITH_TOBACCO: no matches, put back paper. I'll be back in a while.\n");
		sem_post(&paper);
	}
...
void* smoker_paper_routine(void* arg){
	...
	if(sem_trywait(&matches) == 0){
		...
		printf("SMOKER_WITH_PAPER: no matches, put back tobacco. I'll be back in a while.\n");
		sem_post(&tobacco);
	}
	pthread_mutex_lock(&queue_mutex);
...
void* smoker_matches_routine(void* arg){
	...
	if(sem_trywait(&paper) == 0){
		...
		printf("SMOKER_WITH_MATCHES: no paper, put back tobacco. I'll be back in a while.\n");
		sem_post(&tobacco);
	}
	pthread_mutex_lock(&queue_mutex);
